...
def analyze_and_sort_page_elements(page_elements, page_width, page_no):
    """
    מנתח את פריסת העמוד (1, 2 או 3 עמודות) וממיין את האלמנטים בהתאם לסדר הקריאה העברי (RTL).
    זוהי הגרסה הסופית, המטפלת ב-3 עמודות.
    """
    if not page_width or not page_elements:
        return page_elements, []

    # הגדרת קווי הפרדה לשלישים של העמוד
    ...
    for item in page_elements:
        ...
        try:
            ...
            elif item_center > gutter2:
                right_col.append(item)
            elif item_center < gutter1:
    ...
    is_three_columns = (len(right_col) > COLUMN_DETECTION_THRESHOLD and
                        len(middle_col) > COLUMN_DETECTION_THRESHOLD and
                        len(left_col) > COLUMN_DETECTION_THRESHOLD)
    
    is_two_columns = (len(right_col) > COLUMN_DETECTION_THRESHOLD and
                      (len(left_col) + len(middle_col)) > COLUMN_DETECTION_THRESHOLD and
                      not is_three_columns)

    layout_type = "עמודה אחת"
    if is_three_columns:
        layout_type = "3 עמודות"
    elif is_two_columns:
        layout_type = "2 עמודות"
        # במקרה של 2 עמודות, נאחד את העמודה האמצעית עם השמאלית למקרה שההפרדה לא הייתה מושלמת
        left_col.extend(middle_col)
        middle_col = []
    ...
    # שלב 4: הרכבת סדר הקריאה הסופי על בסיס הפריסה שזוהתה
    sorted_body = []
    if is_three_columns:
...
def process_and_structure_data(data):
    ...
    for ref_path in body_children_refs:
        ...
        # אם זה מצביע לקבוצה, נצטרך לטפל בזה בנפרד (הקוד הנוכחי פשוט יותר)
    ...
    for item in all_items_in_body:
        ...
        try:
            ...
            if not any(el.get('self_ref') == item.get('self_ref') for el in pages_data[1]):
...
def convert_docling_json_to_md(json_path, output_path=None):
    ...
    print("  [מבצע תיקוני OCR ומיון גיאומטרי מתקדם]")
...
def main():
    parser = argparse.ArgumentParser(
        description="ממיר קובץ Docling JSON לקובץ Markdown, עם זיהוי פריסה מתקדם.",
